...
export class BebidaService {
  ...
  private bebidasSubject = new BehaviorSubject<Bebida[]>(this.bebidas);
   ...
   */
  obtenerBebidaPorId(id: number): Bebida | undefined {
   ...
   */
  agregarBebida(nuevaBebida: Omit<Bebida, 'id'>): Bebida {
    const bebida: Bebida = {
      id: this.siguienteId++,
      ...nuevaBebida
    };
   ...
   */
  actualizarBebida(id: number, datosActualizados: Partial<Bebida>): Bebida | null {
   ...
   */
  obtenerBebidasStockBajo(): Bebida[] {
   ...
   */
  buscarBebidas(termino: string): Bebida[] {
    if (!termino.trim()) {
      return this.bebidas;
    }
   ...
   */
  filtrarPorPrecio(precioMin: number, precioMax: number): Bebida[] {
    return this.bebidas.filter(bebida => 
